...
class MiniSom(object):
    ...
    def train_batch(self, data, num_iteration, verbose=False, eventFunction = None):
        ...
        for iteration in iterations:
            ...
            self.update(data[idx], self.winner(data[idx]),
                        iteration, num_iteration)
            if eventFunction != None and iteration > 0 and idx == 0:
              if not eventFunction(iteration, data = data, epoch = int((iteration + 1) / (len(data)-0)), minisom = self): break
